<template>
    <div>
        <b-card title="New Airport" style="max-width: 30rem;" class="ml-auto mr-auto mt-3">

            <b-form @submit.prevent="handleSubmitAddAirport">
...
<script>
    ...
    export default {
        ...
        methods: {
            handleSubmitAddAirport() {
                this.message = '';
                if (this.idValidation && this.nameValidation && this.placeValidation) {
                    this.airport.id = this.airport.id.toUpperCase()
                    AirportService.postAddAirport(this.airport).then(
                        response => {
